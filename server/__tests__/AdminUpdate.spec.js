...
describe("Admin update", () => {
  ...
  it("updates username in database when valid update request is sent from user", async () => {
    ...
    const inDBUser = await Admin.findOne({ where: { id: savedUser.id } });
  ...
  it("saves the admin image when update contains image as base64", async () => {
  ...
  it("saves the admin image to upload folder and stores filename to driver  when update has image", async () => {
    const fileInBase64 = readFileAsBase64();
    const savedUser = await addUser();
    const validUpdate = { username: "user1-updated", image: fileInBase64 };
    await putUser(savedUser.id, validUpdate, {
  ...
  it("keeps the old image after user only updates useername", async () => {
    ...
    const userInDb = await Admin.findOne({ where: { id: savedUser.id } });
